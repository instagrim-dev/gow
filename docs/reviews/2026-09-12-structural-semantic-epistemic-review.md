...
| Epistemic: N2a supersession surfacing, prospective comparison | **Supersession views delivered; persuasive episode run remains operator workstream** | v41: typed two-observation episode protocol (`episodes` / `episode_commitments` / `episode_observations` / `episode_revisions`, loop order enforced transactionally, scores code-derived) + `internal/witness` exact Erdős–Straus checker as the first decisive domain-goal outcome mechanism (`newf episode preregister/observe/revise/commit/show`, `docs/episode.md`); N2a current-claim supersession views delivered 2026-09-12: authoritative `records/CURRENT-CLAIMS.md` (claim table with supersession links for N2a/N5/children/joint-crossing/E15), current-claim banners atop the reassessed challenge records (superseded verdict lines flagged in place), and scorecard rows annotated to defer to the current-claim view; running a persuasive comparative episode against a live map, with full cost accounting, is still open operator work |
