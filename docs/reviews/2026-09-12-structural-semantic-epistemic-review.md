...
| Epistemic: N2a supersession surfacing, prospective comparison | **Substrate delivered; episode run + N2a views remain operator workstream** | v41: typed two-observation episode protocol (`episodes` / `episode_commitments` / `episode_observations` / `episode_revisions`, loop order enforced transactionally, scores code-derived) + `internal/witness` exact Erdős–Straus checker as the first decisive domain-goal outcome mechanism (`newf episode preregister/observe/revise/commit/show`, `docs/episode.md`); running a persuasive comparative episode against a live map, with full cost accounting, and the N2a current-claim supersession views are still open operator work |
