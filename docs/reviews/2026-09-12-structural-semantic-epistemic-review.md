...
| Epistemic: N2a supersession surfacing, prospective comparison | **Remediated (loop closed); comparative two-arm study remains future work** | v41: typed two-observation episode protocol (`episodes` / `episode_commitments` / `episode_observations` / `episode_revisions`, loop order enforced transactionally, scores code-derived) + `internal/witness` exact Erdős–Straus checker as the first decisive domain-goal outcome mechanism (`newf episode preregister/observe/revise/commit/show`, `docs/episode.md`); N2a current-claim supersession views delivered 2026-09-12 (`records/CURRENT-CLAIMS.md`, banners, scorecard annotations); **first operator episode run 2026-09-12 against the live M7 map**: `epi_01M2BED4J0GN24NV0P1GJTZRXQ`, preregistered instance rule (p=1000033, first prime ≥10⁶ ≡ 1 mod 24), greedy-local miss (residue shown exactly) → recorded map revision → structurally different global divisor-lattice action → exact hit; `revision_credit: earned` (code-derived); 97 s wall-clock, zero model calls — record: `corpus/experiments/m7-blinded-run/records/episode-001-greedy-vs-global.md`; **two-arm comparison run 2026-09-12** (`corpus/experiments/two-arm-comparison/`, protocol frozen at `d75b4e0` pre-execution): map-ordered arm 20/20 verified witnesses at 20 submissions vs undirected arm 3/20 at 57, frozen decision rule verdict *earned*, with budget-sensitivity and fixed-pool limits stated in RESULT.md; the remaining honest gap is a discovery-scale comparison where the map must find (not merely order) a mechanism family |
