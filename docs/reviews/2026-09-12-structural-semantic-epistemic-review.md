...
| S2 evidence admission / atlas materialization | **Open** — [#19](https://github.com/instagrim-dev/gow/issues/19) | `evaluated_failures` remains a marker, not an admitted observation; needs typed admission rules per observation kind |
...
| S5 typed boundary_delta / concrete projection | **Open** — [#20](https://github.com/instagrim-dev/gow/issues/20) | glossary contract still exceeds `ChallengeResult`'s typed fields; four-record projection chain (structural change → artifact → verification obligation → domain observation) is design-level work |
| Semantic: authored claim form (quantifier/scope) | **Open** — [#21](https://github.com/instagrim-dev/gow/issues/21) | `associationKindForCandidate` still derives universality from measured coverage; the code refuses to invent it (conservative), but an authored `predicate + quantifier + scope + role + assessment context` spec remains future work |
| Semantic: verification subject axis | **Open** — [#21](https://github.com/instagrim-dev/gow/issues/21) | verifier kind/strength lack an annotation/realization/domain-goal subject |
