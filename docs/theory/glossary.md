...
| `Projection` | Translation of a shape-space move into a classical / domain candidate that can be verified. | The typed projection chain (v37): `frontier_proposals` (proposed structural change) → `projection_artifacts` (authored concrete plan; steps with requires/provides tokens) → `projection_obligations` (steps-compose decided by code; domain-realization left open for an external checker) → the evaluation a discharge decision records as its domain observation. See `docs/projection.md` |
...
| `claim_role` | The interpretive standing of a shape claim, recorded separately from shape, regime, and status. | `regularity` / `obstruction` / `enabling_condition` / `boundary_hypothesis` — authored on the persisted claim form (`invariant_claim_forms`, v39) together with `quantifier ∈ universal\|recurrent\|existential` and an authored `scope`; universality is never inferred from measured coverage |
