...
supersedes: CUSTODIAN_PUBLIC_INTERFACE_V4.md; CUSTODIAN_PUBLIC_INTERFACE.md remains the frozen attempt-4 interface
---

# Custodian public interface v5
...
<dispatch-root>/packet/newf --db <dispatch-root>/protected/custodian.db --json review policy \
  --key g1-v5-protected-usefulness --revision 1 \
  --decision-name "protected G1 v5 usefulness progression" \
...
Use this exact cross-stratum allocation. It requires direct execution of both
observation assessors while retaining explicit non-certification controls:

| Stratum | Route and count | Required terminal result |
|---|---|---|
| Applicable (24) | 8 `finite_equivalence`; 4 `finite_instance`; 6 `observed_rate_invariance`; 4 `solved_monotonicity`; 2 `probabilistic_property` | All finite and observed/solved routes use fully bound inputs. The six observed-rate cases contain exactly three `HOLDS_AT_COMPARED_POINTS` and three `REFUTED` results. The four solved cases have verified trace extension; at least two begin with an actually solved instance and all retain it. The probability cases expect `NOT_ASSESSED`; this is a correct terminal nonassessment, not a refusal to run the public route. |
| Inapplicable (16) | 4 `finite_instance`; 6 `observed_rate_invariance`; 6 `solved_monotonicity` | Supply exactly one typed premise defect. Finite-instance cases use only an invalid width, an undeclared expression variable, or a duplicate assignment; never omit `assignments` here. Observed-rate cases mismatch a declared condition against the binding. Solved cases change a high-budget prefix entry, making traces non-nested. Every retained verdict is `INAPPLICABLE` and names the defect. It must never certify the claim. |
| Underspecified (8) | 2 `finite_instance`; 4 `observed_rate_invariance`; 2 `solved_monotonicity` | Omit one real required premise. Finite-instance cases omit `assignments`; observation cases omit `binding` or `observations`. Every retained verdict is `UNRESOLVED` and names that absence. |

This allocation totals 48 cases with the frozen 24/16/8 strata and route
composition 8/10/16/12/2 for finite equivalence, finite instance, observed
rate, solved monotonicity, and probability routing. Do not repurpose a
reservation refusal, malformed JSON rejected before persistence, cancellation,
or storage failure as an inapplicable or underspecified case.
...
### Terminal-state self-check

Before writing the return packet, classify each persisted semantic result with
the preceding mapping. A semantic `INAPPLICABLE`, `UNRESOLVED`, or
`NOT_ASSESSED` result that matches its sealed answer is `completed-valid`, even
when the review ledger calls that result `blocked`. The G1 case state `blocked`
is reserved exclusively for storage failure, process interruption, cancellation,
or a resource reservation refusal. Every reported G1 `blocked` entry must name
one of those infrastructure categories in `blocked_actions`. Never label a
semantic mismatch, wrong sealed expectation, or other semantic issue as G1
`blocked`; stop with `verification_blocked` and record a content-free contract
failure instead. Do not retry a semantic result.
...
`{"schema":"g1-v5-task-manifest/1","entries":[...] }`. It has exactly 48
...
- `stratum` is one of `applicable`, `inapplicable`, `underspecified`;
  ...
  `probabilistic_property`; route counts are exactly 8/10/16/12/2.
...
`{"schema":"g1-v5-answer-manifest/1","entries":[...] }`. It has the same
...
`schema: "g1-v5-expected-answer/1"`, the matching case ID,
...
`g1-v5-case-provenance/1`; values must agree with both manifests.
...
`first_check_not_started`. Use `schema: "g1-v5-pre-execution-seal/1"` and
...
Write `G1_V5_RETURN_PACKET.json` in the return root using this content-free
shape. Replace angle-bracket values only; do not add fields.

```json
{"schema":"g1-v5-return-packet/1","dispatch_id":"<dispatch-id>","release_revision":"<pinned-revision>","executable_sha256":"<pinned-sha256>","policy_id":"<policy-id>","obligation_ids":{"finite-equivalence-route@1":"<id>","finite-instance-route@1":"<id>","observed-rate-route@1":"<id>","solved-monotonicity-route@1":"<id>","probabilistic-routing-route@1":"<id>"},"manifest_identities":{"task_manifest":{"sha256":"<lower-case sha256>","byte_length":<positive>},"answer_manifest":{"sha256":"<lower-case sha256>","byte_length":<positive>},"case_provenance":{"sha256":"<lower-case sha256>","byte_length":<positive>},"pre_execution_seal":{"sha256":"<lower-case sha256>","byte_length":<positive>}},"metadata_identity":{"sha256":"<lower-case sha256>","byte_length":<positive>},"seal_identity":{"sha256":"<lower-case sha256>","byte_length":<positive>},"custody_limitations":["<content-free limitation>"],"blocked_actions":["<content-free action or empty list>"],"stratum_outcome_counts":{"applicable":{"completed-valid":<nonnegative>,"refused-applicable":<nonnegative>,"false-certification":<nonnegative>,"blocked":<nonnegative>,"not-executed":<nonnegative>},"inapplicable":{"completed-valid":<nonnegative>,"refused-applicable":<nonnegative>,"false-certification":<nonnegative>,"blocked":<nonnegative>,"not-executed":<nonnegative>},"underspecified":{"completed-valid":<nonnegative>,"refused-applicable":<nonnegative>,"false-certification":<nonnegative>,"blocked":<nonnegative>,"not-executed":<nonnegative>}},"route_terminal_counts":{"finite_equivalence":{"HOLDS_ON_DECLARED_DOMAIN":<nonnegative>,"REFUTED":<nonnegative>,"UNRESOLVED":<nonnegative>,"INAPPLICABLE":<nonnegative>},"finite_instance":{"INSTANCE_EVIDENCE_ONLY":<nonnegative>,"REFUTED":<nonnegative>,"UNRESOLVED":<nonnegative>,"INAPPLICABLE":<nonnegative>},"observed_rate_invariance":{"HOLDS_AT_COMPARED_POINTS":<nonnegative>,"REFUTED":<nonnegative>,"UNRESOLVED":<nonnegative>,"INAPPLICABLE":<nonnegative>},"solved_monotonicity":{"HOLDS_AT_COMPARED_POINTS":<nonnegative>,"REFUTED":<nonnegative>,"UNRESOLVED":<nonnegative>,"INAPPLICABLE":<nonnegative>},"probabilistic_property":{"NOT_ASSESSED":<nonnegative>}},"observation_coverage":{"observed_rate_applicable":{"holds":3,"refuted":3,"assessor_invoked":6},"solved_monotonicity_applicable":{"verified_extensions":4,"previously_solved_cases":<2..4>,"assessor_invoked":4}},"elapsed_seconds":<nonnegative>,"completion_state":"<completed|interface_unrepresentable|execution_interrupted|resource_exhausted|verification_blocked>"}
```

Each stratum object totals its frozen population (24, 16, or 8). Route terminal
counts total the frozen route composition 8/10/16/12/2, and the observation
coverage section is content-free aggregate evidence of the required applicable
controls. If metadata or a seal was never created because the interface was
terminally unrepresentable, use a zero-byte identity with an all-zero SHA-256
and include the reason in `blocked_actions`; never replace the identity object
with a raw explanation. No raw, case-specific, expected-answer, result, or
score content is allowed. Write `G1_V5_RETURN_PACKET.sha256` containing the
SHA-256 of its exact JSON bytes. The final response reports only the completion
state, both return paths, and that digest.
