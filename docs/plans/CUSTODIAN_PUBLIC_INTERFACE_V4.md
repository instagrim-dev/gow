...
scope: future protected G1 v4 evaluator only
supersedes: CUSTODIAN_PUBLIC_INTERFACE_V3.md; CUSTODIAN_PUBLIC_INTERFACE.md remains the frozen attempt-4 interface
---

# Custodian public interface v4
...
<dispatch-root>/packet/newf --db <dispatch-root>/protected/custodian.db --json review policy \
  --key g1-v4-protected-usefulness --revision 1 \
  --decision-name "protected G1 v4 usefulness progression" \
...
| Inapplicable (16) | 4 `finite_instance`; 6 `observed_rate_invariance`; 6 `solved_monotonicity` | Supply a typed premise defect. Finite-instance cases use a named domain, expression, or assignment defect; observed-rate cases violate the declared comparison binding; solved cases are non-nested. Every retained verdict is `INAPPLICABLE` and names the defect. It must never certify the claim. |
...
`{"schema":"g1-v4-task-manifest/1","entries":[...] }`. It has exactly 48
...
`{"schema":"g1-v4-answer-manifest/1","entries":[...] }`. It has the same
...
`schema: "g1-v4-expected-answer/1"`, the matching case ID,
...
`g1-v4-case-provenance/1`; values must agree with both manifests.
...
`first_check_not_started`. Use `schema: "g1-v4-pre-execution-seal/1"` and
...
score content is allowed. Write `G1_V4_RETURN_PACKET.sha256` containing the
