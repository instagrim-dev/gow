...
scope: future protected G1 v3 evaluator only
...
<dispatch-root>/packet/newf --db <dispatch-root>/protected/custodian.db --json review policy \
  --key g1-v3-protected-usefulness --revision 1 \
  --decision-name "protected G1 v3 usefulness progression" \
...
`{"schema":"g1-v3-task-manifest/1","entries":[...] }`. It has exactly 48
...
`{"schema":"g1-v3-answer-manifest/1","entries":[...] }`. It has the same
...
`schema: "g1-v3-expected-answer/1"`, the matching case ID,
...
`g1-v3-case-provenance/1`; values must agree with both manifests.
...
`first_check_not_started`. Use `schema: "g1-v3-pre-execution-seal/1"` and
...
Write `G1_V3_RETURN_PACKET.sha256` containing the SHA-256 of its exact JSON
