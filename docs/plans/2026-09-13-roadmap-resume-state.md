...
| G2 | A pinned `egg/0.11.0` subprocess now performs bounded extraction from already admitted finite rules; Go independently replays each attributed step and checks source, target, identity, direction, substitutions, and the guard-free scope before the finite endpoint oracle | Development-only engine ingress: proof material and bounded work are checked, but graph/dependency records, withdrawal/rebuild behavior, and measured execution cost are not yet durable | Persist rule dependencies and separate cost ledgers, then execute admit → union → withdraw → rebuild → reassess before claiming the G2 exit |
