...
| X0 / G4-lite | Content-free `g4-lite-pack/1` validation and immutable sealing now freeze the 12/6/6 population, three arm snapshots, shared checker/model/tool/resource identities, calibration identity, run design, endpoint, and funding rule; the development diagnostic remains separately executable | No protected batch or history-value evidence; a matching seal is `PREPARED_NOT_AUTHORIZED` and retains unverified custody | Fresh custodian-held episodes/answers, actual arm/resource records, a concrete execution authorization, and a complete metered grid |
