...
| G3 | `composition-task/1` separates custodian-authored task material from a task-digest-bound `composition-candidate/1` proposer submission; commit and observation preserve the existing independent fidelity/objective split. Content-free `g3-pack/1` seals separate task/answer manifest identities and requires declared met, objective-miss, menu-selection, missing-precondition, and unwarranted-equality coverage. | No independently authored/access-controlled G3 task pack or prospective external evaluation; the new metadata seal retains declared custody without certifying it. | Dispatch a fresh custodian to author and seal the protected family, then independently execute and grade every declared route without relabeling development results. |
