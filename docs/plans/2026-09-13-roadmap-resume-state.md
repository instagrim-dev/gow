...
| X0 / G4-lite | Content-free final `g4-lite-pack/2` validation and immutable sealing bind custodian-authored manifest identities to the 12/6/6 population, three arm snapshots, shared checker/model/tool/task-directed-resource identities, calibration identity, exact run-summed scoring semantics, run design, endpoint, and funding rule | No protected batch or history-value evidence; a matching seal is `PREPARED_NOT_AUTHORIZED` and retains unverified custody | Fresh custodian-held episodes/answers, actual arm/resource records, a concrete execution authorization, and a complete metered grid; the delivered executor supports the disclosed deterministic one-run design only; see `2026-09-14-g4-lite-dispatch-readiness.md` |
...
| G4 | V1 remains `invalid` for frozen-controller identity nonconformance. The retained open calibration at `b6f3e4a` had H0=24, H1=6, HG=24; matching runtime preflight passed at `af9d2cf`. Fresh V2S then had a valid content-free chain but saturated H0=24, H1=24, HG=24 and was graded `INCONCLUSIVE_INCOMPLETE`. | Neither calibration/preflight nor V2S earns a population-level or shaping-value claim. V2S is not invalid, but its ceiling has no discriminating outcome. | Revise the open task-generation/resource procedure; retain a new non-ceiling calibration; then create a fresh custodian-held successor with complete costs, authorization, and independent grading. |
