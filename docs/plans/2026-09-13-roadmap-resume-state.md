...
| G3 | `composition-attempt/1` commits a finite-expression residual, capability requirement, preconditions, two-schema constructed trace, and action-menu exclusion before outcome observation; `composition observe` independently replays semantic fidelity and measures the original execution-cost objective separately. Development fixtures cover faithful, menu-selection, missing-precondition, and unwarranted-equality routes. | No independently authored/access-controlled G3 task pack or prospective external evaluation; task authoring provenance is recorded but not certified. | Custodian-authored protected task family and all external-evidence failure routes, then evaluate without relabeling development results. |
