...
| X0 / G4-lite | Content-free final `g4-lite-pack/2` validation and immutable sealing now bind custodian-authored manifest identities to the 12/6/6 population, three arm snapshots, shared checker/model/tool/task-directed-resource identities, calibration identity, exact run-summed scoring semantics, run design, endpoint, and funding rule; the development diagnostic remains separately executable | No protected batch or history-value evidence; a matching seal is `PREPARED_NOT_AUTHORIZED` and retains unverified custody | Fresh custodian-held episodes/answers, actual arm/resource records, a concrete execution authorization, and a complete metered grid |
