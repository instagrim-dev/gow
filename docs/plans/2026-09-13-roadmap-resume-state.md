...
| X0 / G4-lite | Content-free final `g4-lite-pack/3` validation and immutable sealing bind custodian-authored manifest identities to the 12/6/6 population, frozen generation procedure, executable-bound `/2` arm snapshots, shared checker/model/tool/task-directed-resource identities, calibration identity, exact run-summed scoring semantics, run design, endpoint, and funding rule | No protected batch or history-value evidence; a matching seal is `PREPARED_NOT_AUTHORIZED` and retains unverified custody | Fresh custodian-held episodes/answers under the frozen procedure, actual arm/resource records, a concrete execution authorization, and a complete metered grid; historical `/2` pack records and `/1` arm identities remain readable only as historical evidence; see `2026-09-14-g4-lite-dispatch-readiness.md` |
| G2 | Bounded `egg/0.11.0` extraction now has independently replayed proof material, endpoint replay, separate extraction/execution-cost ledgers, and immutable SQLite graph/dependency events; the compiled-engine test proves admit → union → rebuild → withdraw → rebuild → reassess | Completed only for the guard-free finite seed language and AST-size objective; this is an equivalence optimizer capability, not evidence of learned geometry or broader conditional-rule support | G3-approved composition slice and task evaluation |
| G3 | `composition-task/1` separates custodian-authored task material from a task-digest-bound `composition-candidate/1` proposer submission; response-independent met/miss objectives, pre-execution sealing, and the post-execution lineage receipt prevent route substitution. | Fresh V6 replay passed all 12 artifact-chain checks, including five route replays and exact return bindings. The result is limited to `agent-sealed/v1`: custody independence and external authorization remain unverified by the files. | G4 design/freeze is the next separate authority decision; do not promote the bounded G3 replay into broader research, spending, or publication authority. |
| G4 | V1 remains `invalid` for frozen-controller identity nonconformance. The retained V2S chain is `INCONCLUSIVE_INCOMPLETE` at H0=24, H1=24, HG=24. The frozen multi-choice open procedure has an exact-vector successor retaining 6/6/6 and 24/6/24 responses across its predeclared vectors; `/3` binds that procedure and executable-bound `/2` arm identities to a fresh protected execution. | Neither calibration/preflight nor V2S earns a population-level or shaping-value claim. V2S is not invalid, but its ceiling has no discriminating outcome. | Fresh custodian-held authoring under the frozen procedure, complete costs, authorization, and substantive independent grading. |
