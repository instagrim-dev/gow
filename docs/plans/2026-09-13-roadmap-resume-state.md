...
| X0 / G4-lite | Content-free final `g4-lite-pack/3` validation and immutable sealing bind custodian-authored manifest identities to the 12/6/6 population, frozen generation procedure, three arm snapshots, shared checker/model/tool/task-directed-resource identities, calibration identity, exact run-summed scoring semantics, run design, endpoint, and funding rule | No protected batch or history-value evidence; a matching seal is `PREPARED_NOT_AUTHORIZED` and retains unverified custody | Fresh custodian-held episodes/answers under the frozen procedure, actual arm/resource records, a concrete execution authorization, and a complete metered grid; historical `/2` records remain readable; see `2026-09-14-g4-lite-dispatch-readiness.md` |
...
| G4 | V1 remains `invalid` for frozen-controller identity nonconformance. The retained V2S chain is `INCONCLUSIVE_INCOMPLETE` at H0=24, H1=24, HG=24. A frozen multi-choice open procedure now retains 6/6/6 and 24/6/24 responses across its predeclared vectors; `/3` binds that procedure to a fresh protected execution. | Neither calibration/preflight nor V2S earns a population-level or shaping-value claim. V2S is not invalid, but its ceiling has no discriminating outcome. | Fresh custodian-held authoring under the frozen procedure, complete costs, authorization, and substantive independent grading. |
