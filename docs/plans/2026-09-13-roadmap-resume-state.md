...
| G4 | The private V1 dispatch is retained `invalid` for frozen-controller identity nonconformance; its saturated 24/24/24 completion counts are a ceiling diagnostic. A separately retained open development calibration at `b6f3e4a` has H0=24, H1=6, HG=24 at an arm-blind median of two expansions. | The non-ceiling calibration is construction-exposed development evidence only; no new population-level or shaping-value claim. | Runtime-identity preflight for the chosen successor release, then a successor design with fresh families, complete costs, custody, and authorization. |
