...
| **10. Freeze arXiv v1** | Tagged paper/repo release | Every empirical number traceable to an immutable artifact **and consistent with it**; a commit-specific PASS recorded under review gate #16; exact source package validated | 🔄 **Gate #16 finding 4 (evidence deposit) resolved by public repository; ready for a PASS at the next reviewed commit pending author sign-off on the exact submission bundle.** The repository was flipped to public visibility at `github.com/instagrim-dev/gow` under the Apache License 2.0 (`LICENSE` file added at this commit); a full-history `gitleaks` scan across 168 commits reported no leaks. The manifest'd 65 evidence-supplement files (`corpus/evidence-supplement/manifest.json`) are now accessible at commit-anchored URLs in the public tree, with per-file SHA-256 integrity check; the Evidence Availability section of `paper/geometry-of-work.tex` was rewritten to point at the public repository and describe the hash-verification path. This is a repository-based deposit; a canonical-archive DOI (Zenodo/OSF) is not obtained but is compatible with a later strengthening step. The three-lane independent-quorum review at `a98c93a` remains the recorded substitute for the uninvolved-human-reviewer criterion (still explicitly model-judgment, not peer review). The human author retains publication authority and controls the actual arXiv upload, license selection (`CC BY 4.0` recommended for the paper), and endorsement. **The `newf` software `v1.0.0` tag is unchanged and remains independent of this row.** |
