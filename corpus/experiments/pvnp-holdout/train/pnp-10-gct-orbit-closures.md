...
**Era:** 2001. Pre-cutoff. Anchor: Mulmuley–Sohoni, *Geometric complexity
theory I*, SIAM J. Comput. 31(2), 2001.

GCT is the train atlas's most radical re-representation: recast permanent vs
...
Its recorded status at the cutoff is a **program, not a result**: no
...
open problems themselves. The structural lesson the atlas keeps: escaping
both walls is *possible in posture* (rare, structured, non-black-box), but the
only occupant of that space at cutoff traded the walls for open problems at
least as hard, and still analyzed the **model side** (orbit closures of the
computing objects), never the algorithm side.
  ...
  "approaches": [
    {
      ...
      "outcome": {
        ...
        "notes": "Occupies the mapped escape space in posture only; still analyzes the model side, never the algorithm side."
      },
      "support": [
