...
| **Recommended next action** | **Revised 2026-09-12 (third revision) after the code-owned challenge campaign confirmed H2 extends to challenge probes and revealed the protocol delta:** (a″) closed — the code-owned challenge campaign has been run on all 3 M7 invariants; all remain `surviving`; L3 sits at threshold with support=2. Remaining paths in priority: **(a‴)** Mine an enum-axis or Not-form invariant on the M7 failure population — same as prior (a′) but now with clearer motivation: the code-owned protocol will produce decisive verdicts on enum/Not predicates whereas Contains-preserves stays at `unconfirmed`. **(c)** Run `newf evidence admit` against the persisted joint-crossing proposal `fpr_01M2B919...D2YB1` under S2's rules. Predicted verdict: `structural-claim-failure` = never admissible. Testable end-to-end now. **(e)** Design-space observation to record but NOT execute here: C6 (support-set variability) could be added to the code-owned challenge protocol cheaply — the predicate is `(distinct_family_support - threshold) / support ≤ ε` and is derivable from data already persisted. L3's threshold=support case is exactly what such a probe would flag. Not a change proposal here; recording as a design-space observation for downstream authority. **(b)** Materialise a Pilot-004-specific SQLite fixture — distinct research campaign, unchanged. Separately: OQ2 remains deferred; N5-child-1 remains blocked on predicate formalization; N2b es-07 inclusion correction still outstanding; atlas `breaks` field inconsistency as an atlas-quality task. |
