...
| **Recommended next action** | **Revised 2026-09-12 (fourteenth revision) after (witness-scope) fresh SGO on all 10 M7 proposals:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope) closed for this iteration. **The witness-occurrence attribution obligation has been scope-checked at full-corpus granularity: 0/10 proposals carry an in-scope obligation, 0/3 evaluations use the witness path. This is NOT a discharge of the obligation — it is a bounded observation that discharge requires operator-supplied proposal input (candidate-specific `(n,x,y,z)` claim with provenance) that no ranker, admission rule, or verifier can synthesize.** Remaining priorities are research-execution, corpus-materialization, or design-space observations awaiting downstream authority: **(l)** Cross-domain replication — materialize pilot-005-relational or another non-ES pilot corpus. **(k-full)** Historical pilot re-execution — multi-loop, atlas-quality re-normalization + operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families (design-space; needs authorization; **now concretized**: the current generator produces 0/10 witness-carrying proposals, so any move toward concrete-witness generation is a strictly new capability). **(q)** Authoring-discipline documentation — operator-judgment signal requiring calibration, not a code-owned discriminator. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (paper-space "correlation vs causal obstruction" probe absent from code-owned protocol; never run on M7's L1/L3/L4). **Witness-occurrence attribution obligation** (scope-checked; not discharged; requires operator-supplied concrete tuple + provenance). **C8 obligation** (label does not resolve against the paper-space 7-probe protocol or any locatable review artifact; awaiting reviewer clarification of intended scope). Pausing exploratory code-change work is not evidence of end-to-end conformance on those obligations. **The corpus of open code-change paths for this pilot iteration is small; the corpus of open review-integration obligations is not.** OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
