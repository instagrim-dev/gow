...
| **Recommended next action** | **Revised 2026-09-12 (eighteenth revision) after (l-coh-mechanism-test) upgraded correction 7 to CMA-tier:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh), (l-coh-mechanism-test) closed for this iteration. **Correction 7 is now CMA-tier at the probe-verdict level; the other eight (l-coh) corrections remain unchanged.** Remaining priorities (agent-tractable are marked; others need authorization): **(l-population-validation)** ⭐ agent-tractable: discharge the population-query prerequisite from (l-coh) correction 6 — verify the (l-coh) arithmetic used the eligible families the miner and challenger actually operated on, not merely reachable signatures. Highest remaining agent-tractable priority. **(l-extension)** Author a third truly-independent corpus in a non-ES, non-P-vs-NP domain to move any research-usefulness claim beyond n=2 lineages. Separate experiment from the code-mechanism test — needs corpus authoring authorization. **(k-full)** Historical pilot re-execution — needs operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families — needs authorization. **(pilot-005-freeze)** Operator authorization on six open items. **(q)** Authoring-discipline documentation. **(coherence-metric-pipeline)** Turn (l-coh) measurement into pipeline capability — needs authorization; less compelling now that correction 7 is code-verified. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
