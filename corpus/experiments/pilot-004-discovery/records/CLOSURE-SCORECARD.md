...
| **Recommended next action** | **Revised 2026-09-12 (sixth revision) after (a‴-B) proved H2 escape end-to-end and revealed H2⊥H3 orthogonality:** Paths (a″), (c) closed. Path (a‴) partially closed — (a‴-B) executed; (a‴-A) `--proposals-file` on mine still open as a lower-priority variant. New priorities: **(g)** Escape H3. Now that H2 is known escapable but H3 blocks even H2-escaped proposals, the H3 gate is the next natural target. Two sub-branches: **(g-A)** A deterministic-check verifier that consumes target-violation verdicts as evidence (a proposal that decisively violates its target invariant plausibly earns a `partial_failure` or `partial_success` verdict at evaluation, but the current verifier chain doesn't consult target verdicts). Small design change; requires care about what a target-violation MEANS for a proposal outcome. **(g-B)** A stronger model-judgment verifier that can produce `failure`/`partial_failure` on posture-decisive proposals — trivial with fixture, less trivial with real judgment. **(h)** Re-challenge `equals(construction, constructive)` after examining whether the c6 counterexample is atlas-truthful (SGO check: does es-06 or es-10 actually have `posture.construction=constructive`? if the classifier assigned constructive incorrectly, weakening was premature). Atlas-quality task. **(e)** Design-space observation about C6 (unchanged; recorded, not authorised as change). **(b)** Materialise a Pilot-004-specific SQLite fixture — distinct research campaign, unchanged. **(f)** Design-space observation about the three-gate H1/H2/H3 ratchet — now upgraded: **H2 and H3 are orthogonal gates**, escaping H2 does not escape H3. Recording this correction. **(a‴-A)** Lower priority — `--proposals-file` on mine would give operators direct control over predicate shapes but (a‴-B) already produced the target enum-axis emissions organically. Separately: OQ2 remains deferred; N5-child-1 remains blocked on predicate formalization; N2b es-07 inclusion correction still outstanding; atlas `breaks` field inconsistency as an atlas-quality task. |
