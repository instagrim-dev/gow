...
| **Recommended next action** | **Three paths, in priority order: (a) Materialise a live Pilot-004 SQLite corpus fixture. This is the highest-leverage remaining mechanical task: it would unblock `newf frontier generate` end-to-end, let the joint-crossing falsification flow through `newf evidence admit` under S2's typed observation-kind rules (structural-claim-failure: never admissible — the correct verdict), and let future E15-shape corpus-hygiene observations be recorded in the pipeline's own state rather than as markdown records. (b) Address the N2b es-07 inclusion scope correction (OQ1 resolved 2026-09-11 but the correction to E03/E05/E07 group memberships remains outstanding — analogous to what the E15 correction just did for E15/N4). Bounded, atlas-verifiable. (c) Challenge the sharper `N2a-child-2'` form (enumerates admissible reciprocity layers). Substantive testable claim from the prior weakening; genuine risk of returning to the reciprocity-axis diminishing-returns pattern. Separately: OQ2 remains deferred; N5-child-1 remains blocked on predicate formalization. Corpus-hygiene follow-on from the E15' challenge: the frozen atlas's `breaks` field is inconsistently discriminating between outcome_class values — either mark the field as exhaustive/non-exhaustive per record, or normalise the field so its content correlates with outcome_class. Out of the pipeline's invariant scope but worth carrying forward as an atlas-quality task.** |
