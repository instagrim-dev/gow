...
| **Recommended next action** | **Revised 2026-09-12 (fifth revision) after the (a‴) survey confirmed the enum-axis escape hatch:** Path (a‴) refined into two authorization-gated code-change forks: **(a‴-A)** Add `--proposals-file` to `newf invariants mine` (mirrors `frontier generate --proposals-file` shape; small; testable in one loop). **(a‴-B)** Extend `DerivingFixtureInvariantMiner.Mine` to emit `OpEquals(<posture>, value)` where failure-side count ≥ MinSupport AND failure prevalence > success prevalence (small; will bump miner_version reuse key). Either fork enables mining C1 `Equals(locality, local)` at support=3 with 100% failure-side specificity on M7, then testing whether a wire proposal against C1 reaches `verdict: violates` at frontier ranking (H2 escape prediction) and `verdict: failure/partial_failure` at evaluation (H3 escape prediction). **(e)** Design-space observation about C6 (recorded, not authorised as change; remains for downstream authority). **(b)** Materialise a Pilot-004-specific SQLite fixture — distinct research campaign, unchanged. **(f)** Design-space observation about the three-gate H1/H2/H3 ratchet (recorded 2026-09-12). Separately: OQ2 remains deferred; N5-child-1 remains blocked on predicate formalization; N2b es-07 inclusion correction still outstanding; atlas `breaks` field inconsistency as an atlas-quality task. |
