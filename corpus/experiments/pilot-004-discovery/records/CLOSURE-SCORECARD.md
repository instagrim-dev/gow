...
| **Recommended next action** | **Revised 2026-09-12 (twenty-first revision) after (m7-emission-nondivergence) retracted the (m7-emission-divergence) task and added a live real-data cross-check for correction 7:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh), (l-coh-mechanism-test), (l-population-validation), (source-lineage-check), (m7-emission-nondivergence) closed for this iteration. **Correction-7 evidence now sits on three tiers: code inspection, synthetic fixture, live real-data instance. Correction 5 ES-side discharged; correction 6 arithmetic-verified.** Remaining priorities: **(l-extension)** ⭐ agent-tractable with authoring capacity: author a third truly-independent corpus in a non-ES, non-P-vs-NP domain, with a sha256 set disjoint from both known lineages. This is now the strongest remaining research-usefulness path. **(source-lineage-admission-gate)** design-space: turn `(source-lineage-check)`'s sha256-diff into a pipeline corpus-onboarding gate. Needs authorization. **(es-pvnp-independence)** deferred: cross-corpus independence beyond raw source bytes. **(weaken-repair-policy)** ⭐ agent-tractable design-space: `equals(construction, constructive)` on M7 was weakened by a single preserving success. What is the pipeline's designed response — is there a split/repair route that a `weaken` verdict opens? Inspection question, not a new experiment. **(k-full)** Historical pilot re-execution — needs operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families — needs authorization. **(pilot-005-freeze)** Operator authorization on six open items. **(q)** Authoring-discipline documentation. **(coherence-metric-pipeline)** Turn (l-coh) measurement into pipeline capability — less compelling given corrections 5-ES, 6, 7 are all discharged/upgraded. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
