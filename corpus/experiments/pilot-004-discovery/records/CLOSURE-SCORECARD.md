...
| (witness-scope) 2026-09-12 — full-corpus SGO on witness-occurrence attribution: no in-scope obligation found; **NOT a discharge** | SGO + CMA. Bounded scope check on all 10 M7 frontier proposals in `.newf/m7/newf.db` at HEAD `aa3c4a3`. Generalizes and confirms (g-scoped)'s finding (which surveyed only three proposals) by direct enumeration. **Proposal-content survey (SGO, verbatim)**: 3 baselines are family-restatements; 1 is generic re-representation; 3 are B3 "global coupling object" abstract structural claims; 2 are mechanism-level claims (joint-crossing delta-method + Brauer-Manin descent) with prospective falsification steps but zero in-proposal tuples; 1 is the H2-escape pipeline-mechanical test. **Zero proposals** carry an explicit `(n, x, y, z)` integer 4-tuple with provenance. The one occurrence of the word "witnesses" (in `fpr_...D2YB1`) is metaphorical usage for "instances-of-representation the mechanism would produce," not a concrete tuple. **Evaluation-history survey (SGO, aggregate)**: 3 evaluations exist, ALL at `verifier_kind = model-judgment`, ALL at `verification_strength = single-model-judgment`, ALL at `verdict = verification_blocked`. Zero evaluations at `verifier_kind = domain-witness` or `verification_strength = reproducible-computation`. The pipeline's F2 occurrence-attribution machinery has never been exercised on M7. **What discharge would require**: operator-input work — either (a) a wire proposal containing an explicit `(n,x,y,z)` decisive-strength domain-goal claim with provenance, or (b) an operator attaching a witness tuple to an existing proposal via a matching generation-occurrence membership. Neither is agent-tractable; neither is a code-change concern. **Design-space observation** (recorded, not discharged): the generator↔verifier mismatch surfaced in (g-scoped) gains concreteness — 0/10 proposals in the current corpus are witness-carrying, so any move toward concrete-witness generation would be a strictly new capability, not a bias adjustment. **H3 gate status: preserved (unchanged).** No admission rules changed, no new evaluations authored, no proposal inputs constructed. **Clarification appended after concurrent-writer discovery**: reviewer's C7/C8/witness-occurrence attribution terminology maps to a code-owned review-recipe framework (`internal/pipeline/review.go`, `review_coverage.go`, `review_c1c8_disposable_test.go`, recipe `docs/reviews/prompts/recipes/assessment-admission-decision.md`) — C7 there is "historical replay must not displace compatible current context" (NOT paper-space "correlation vs causal"); C8 there is "derived coverage projection" (previously a recorded gap the concurrent writer's `review_coverage.go` appears to close). The scope check is a valid SGO but investigates an adjacent question. Record: `records/witness-occurrence-scope-check.md`. |
| (l) 2026-09-12 — cross-domain replication executed on P vs NP holdout corpus: **Reading B (partial replication)** | SGO + PE. Fresh isolated `.newf/pvnp/newf.db` at HEAD `324ec5d`. Ingested 12-file pvnp-holdout train (adversarially-decontaminated per `008b0c2`), ran normalize → signature → cluster → failure-space (12 singletons, 5 failure + 7 partial_success), mined with `--emit-posture-axes` (a‴-B flag), challenged with `--all`. **Miner emitted 3 enum-axis candidates**: `equals(construction, existential)` support=2, `equals(uncertainty, deterministic)` support=4, `equals(locality, global)` support=3. All 3 persisted with `association_status=contrast_observed`. **All 3 transitioned to `weaken`** under `success-preserving` probe with `contrast-collapse` delta — success-side preservation cross-cuts posture-axis membership. **Comparison to (i)**: ES corpora produced `equals(locality, local)` at `surviving` with 100% failure-side specificity (0/5 success prevalence); pvnp's `equals(locality, global)` at same support=3 has 3/7 success prevalence, driving `weaken`. **Predeclared Reading B obtained** per `records/l-pvnp-replication-predeclaration.md`; Reading A (surviving) not obtained; Reading C (non-replication) not obtained; Reading D (pipeline failure) not obtained. **H3 gate status: preserved (unchanged).** No admission rules changed, no verifier tier changes, no policy directives, no wire schema fields. Record: `records/l-pvnp-replication-result.md`. **Warrant narrowed 2026-09-12 by (l-coh) reviewer correction**: (l)'s original "failure-population coherence in posture-axis space" PE claim was overstated. The statistic used (max between-outcome prevalence difference) measures outcome discrimination, not within-failure coherence. The likelier explanation for the ES-vs-pvnp `surviving`-vs-`weaken` distinction is a code-level property of the `success-preserving` challenge probe — "how the challenge policy converts corpus composition into lifecycle status," per reviewer wording — NOT a research-tier claim about domain-general coherence. Direction (ES > pvnp on maxDiscrim, ratio 2.5×) is consistent with the earlier interpretation; magnitude and mechanism claims are retracted. See (l-coh) entry below for the full narrowing. |
| (l-coh) 2026-09-12 — coherence measurement: COH-A numerical conditions not met; **DISPOSITION NARROWED after reviewer correction** | SGO + CMA + PE, warrant narrowed post-reviewer-correction (record `records/l-coh-measurement-result.md` rewritten with "Corrected disposition" as authoritative section; earlier sections retained for lineage with retraction markers). **Corrected disposition (reviewer wording, adopted)**: *"COH-A's predeclared numerical conditions were not met. ES had greater maximum posture-axis outcome discrimination than P-vs-NP in the reported tables, but this does not establish within-failure coherence, and the alternative reading rules do not uniquely classify the result. The four ES databases should not be counted as four independent research replications; source-level lineage and population-query validation remain necessary. The zero-cell observation suggests a conditional explanation through the existing success-preserving challenge rule, not an unconditional law of invariant survival. Its implementation implications can be tested directly; its usefulness for research requires separate prospective evidence."* **Specific corrections applied**: (1) "falsified" mis-describes the statistic — the numerical conditions were not met, that is not the same thing. (2) Outcome discrimination ≠ within-population coherence; every failure and success could share a value (perfect homogeneity, zero discrim). The 2.5× ratio is arithmetically correct; it does not mean ES is "2.5× more coherent." (3) **COH-B classification WITHDRAWN** — the predeclaration is not sufficiently defined to classify the result; adjudication vs COH-C is underdetermined. (4) The predeclaration's threshold-robustness assertion ("≥ 0.4 preserves the qualitative pattern") is itself falsified by the tables: at 0.4, M7=1 axis-value qualifies, pvnp=0. Frozen 0.5 calculation stands; robustness claim does not. (5) **"n=2 truly independent" WITHDRAWN.** Defensible replacement: "five database instances represent two substantive corpus lineages; four ES instances do not provide four independent research replications; independence between the two corpus constructions has not been established." Identical histograms do not prove identical underlying observations; tripled counts do not identify the cause. (6) Population-query validation is a prerequisite this record did not discharge — family prevalence must count eligible families used by miner/challenger, not every persisted signature. (7) **The zero-cell "iff" WITHDRAWN.** Correct code-derivable statement (from `VerifySuccessPreserving` inspection): *"among emitted, support-valid candidates with comparable resolved populations, absence of a preserving success prevents this particular weakening route. Overall survival additionally depends on the remaining challenge outcomes and claim semantics."* This is code-level, not requiring third-corpus evidence. "No preserving success" is necessary but not sufficient for `surviving`. **UPGRADED 2026-09-12 to CMA-tier** by (l-coh-mechanism-test) — see entry above. (8) **Attribution correction on the numerical minimum**: pvnp's `minSuccessPrev = 0.286` was earlier attributed to `equals(uncertainty, probabilistic)`, which fails BOTH miner emission gates (fc=1<2; fPrev<sPrev). Correct emission-eligible attribution: `equals(construction, existential)` at the coincidentally-identical 2/7 = 0.286. M7's attribution to `equals(locality, local)` at 0.000 was already correct. (9) **Post-hoc epistemic-prohibition framing WITHDRAWN.** Conflated discovery timing, evidence type, and strength of support. Post-hoc observations can be valid code-level deductions; prospective replication tests research usefulness, not the deductive link. **The reviewer's overarching frame**: *"The most important remaining distinction is between discovering structure in research and rediscovering the decision rule that labels that structure `surviving`."* The earlier `minSuccessPrev` observation is probably an instance of the latter. **Concrete next test (reviewer-suggested, NOT executed)**: two singleton-family fixtures with identical discrimination but different sPrev (A: 5/10 vs 0/10; B: 8/10 vs 3/10; both discrim=0.5) — success-preserving probe should distinguish them despite identical discrimination, testing the code-mechanism explanation directly. **H3 gate status: preserved (unchanged).** No admission rules changed, no verifier tier changes, no policy directives, no wire schema fields. |
| (l-coh-mechanism-test) 2026-09-12 — controlled fixture test: **Reading MECH-DISTINGUISH obtained; correction 7 upgraded to CMA-tier** | CMA. Predeclared in `records/l-coh-mechanism-test-predeclaration.md`. Test committed as `internal/invariant/success_preserving_discrim_test.go`. Constructs two `[]Family` fixtures with identical `fPrev-sPrev` discrimination (+0.500) but different sPrev(X) — Fixture A (5 fail/X, 5 fail/Y, 0 succ/X, 10 succ/Y): fPrev=0.5, sPrev=0.0; Fixture B (8 fail/X, 2 fail/Y, 3 succ/X, 7 succ/Y): fPrev=0.8, sPrev=0.3. All members are fully-resolved singleton families (`SetFieldCompleteness[FieldPreserves]=complete`). **Predeclared MECH-DISTINGUISH obtained** on all six conditions: Fixture A → `Outcome=completed_negative, Confirmed=false, Delta=nil, Evidence=[]`; Fixture B → `Outcome=confirmed, Confirmed=true, Delta.Kind=contrast-collapse, len(Evidence)=3 all Kind=success_family`. Correction 7's statement — *"among emitted, support-valid candidates with comparable resolved populations, absence of a preserving success prevents this particular weakening route; overall survival additionally depends on the remaining challenge outcomes and claim semantics"* — is now **CMA-tier at the probe verdict level**. The narrower scope is deliberate: this establishes how `VerifySuccessPreserving` returns different verdicts based on presence/absence of a preserving success family, in the fully-resolved singleton case. It does NOT establish "no preserving success ⟹ overall `surviving`" (campaign can still be weakened/falsified by other probes), does NOT establish research-usefulness of zero-cell selection (requires prospective corpus evidence — separate from this code-mechanism test), and does NOT discharge (l-population-validation) on the actual SQLite arithmetic. Reviewer's overarching frame reinforced: this test is squarely on the "decision rule labeling structure `surviving`" side, not the "discovering structure in research" side. **H3 gate: preserved (unchanged).** No admission rules, no verifier tiers, no policy directives, no wire schema fields changed — pure regression-test artifact. Toolchain gates: `gofmt` clean, `go build ./...`, `go test ./...` all green. Record: `records/l-coh-mechanism-test-result.md`. |
| (l-population-validation) 2026-09-12 — miner-eligible arithmetic verified; **(l-coh) correction 6 discharged**; one narrative correction to data-provenance note | SGO (deterministic recomputation using `derivePostureAxisProposals` semantics). For each of 5 databases, selected latest `cluster_run` per problem, projected `mechanism_clusters` representative signatures, filtered `outcome_mixed`/`unknown` outcomes, filtered redundant-only clusters, filtered `unknown` posture values — reproduced (l-coh) tables. **Result**: M7 table identical (fc/sc counts match); pvnp table identical; **all four ES corpora (M7, pilot-001, pilot-002, pilot-003) produce identical miner-eligible tables** under the corrected query. **Correction to earlier (l-coh) narrative**: pilot-003's counts are NOT 3× M7 under the miner-eligible query — they equal M7 exactly. The "3×" observation was an artifact of not restricting to the latest cluster_run per problem; pilot-003 has 2 `cluster_runs`, the others 1. The narrative correction has been applied to `records/l-coh-measurement-result.md`'s data-provenance section. **Consequences**: (l-coh) correction 6 discharged — the numerical prevalences count the correct population. Correction 5 unchanged — identical marginals across 4 corpora is stronger consistent-with-shared-lineage SGO, not proof of shared lineage; source-level validation remains open. Corrections 1-4, 7-9 unchanged. Persisted candidates from the actual (l) miner runs are exactly the emission-eligible set from the verified tables. **H3 gate: preserved (unchanged).** No admission rules changed, no verifier tiers, no policy directives, no wire schema fields. Pure SGO validation via SQL. Record: `records/l-population-validation-result.md`. |
| (source-lineage-check) 2026-09-12 — **four ES databases share 12/13 source-snapshot sha256 byte-for-byte**; (l-coh) correction 5 ES-side residual discharged | SGO (deterministic SQL comparison across `.newf/{m7,pilot-001,pilot-002,pilot-003}/newf.db`). Compared five fingerprint layers pairwise across the four ES DBs. **Source layer (sha256 + logical_name)**: 12 shared byte-for-byte across all four DBs; M7 has 1 unique file (`es-target-affine-lattice-linear-forms.attested.md`, M7's target problem). **Signature/cluster/input-set fingerprint layers**: differ across DBs, but this reflects pipeline-internal versioning (schema_version, vocabulary_version, run/invocation identity) rather than source-domain differences. **Problem organization layer**: each pilot uses its own slug convention. **Verdict**: the "identical miner-eligible marginals across 4 ES corpora" observation from (l-population-validation) is now explained at CMA-tier — the four DBs ingested the same physical training corpus. **(l-coh) correction 5 ES-side residual DISCHARGED**: positive source-byte evidence that the four ES instances are not four independent research replications. The reviewer's defensible-replacement wording ("four ES instances do not provide four independent research replications") is now supported by concrete sha256 evidence. **ES-vs-pvnp independence residual remains open**: they clearly use different source corpora (M7's ES material vs pvnp's barrier-program corpus), but "independence" in the stronger research sense (mutually informative, non-overlapping mechanism spaces, etc.) requires more work than a sha256 diff. **Practical downstream implication**: for any future (l-extension) work authoring a third truly independent corpus, the source-lineage layer now has a concrete admission criterion — the new corpus's sha256 set must be disjoint from both the ES sha256 set and the pvnp sha256 set. **H3 gate: preserved (unchanged).** No admission rules changed, no verifier tiers, no policy directives, no wire schema fields. Pure SGO via SQL. Record: `records/source-lineage-check-result.md`. |
| (m7-emission-nondivergence) 2026-09-12 — **NON-ISSUE retracted**; both posture-axis candidates were emitted; correction 7 gains a live real-data cross-check | SGO on `.newf/m7/newf.db`, full join across `invariant_revisions × candidate_invariants × invariant_predicates × invariant_current_state × invariant_challenges × invariant_state_transitions`, no state filter. **Finding**: M7's `(a‴-B)` miner (Revision 2 at 2026-09-12T18:03Z, `candidate_count=5`) emitted BOTH posture-axis candidates: `equals(locality, local)` ord=2 → surviving; `equals(construction, constructive)` ord=4 → weaken. Earlier framing of "only surviving persisted" was an artifact of filtering by `state='surviving'`. **Probe-level detail**: on both candidates the `known-counterexample: unconfirmed` and `bias-critique: unconfirmed` probes return identically; the differentiating signal is `success-preserving` — `unconfirmed` ("all 5 eligible success-side member(s) decisively checked; none preserves") on the sPrev=0 candidate, `confirmed` ("1 success-preserving family(ies)") on the sPrev=0.2 candidate. The `contrast_violating_num / contrast_eligible_den` columns (5/5 and 4/5 respectively) reconcile exactly with `(l-population-validation)`'s eligible-family table. **Consequence for correction 7**: still CMA-tier from `(l-coh-mechanism-test)`; not further upgraded (CMA is the ceiling for a code-derivable relationship). But this loop adds a **third orthogonal source of evidence** — the same pattern the synthetic fixture test produced synthetically appears in real ES corpus data on the same miner run. Correction 7 is now supported by: (1) code inspection [reviewer], (2) synthetic fixture test [(l-coh-mechanism-test)], (3) live real-data instance [this loop]. **Retraction**: the `(m7-emission-divergence)` next-action task is retired as a non-issue. **H3 gate: preserved (unchanged).** No admission rules changed, no verifier tiers, no policy directives, no wire schema fields. Pure persistence-layer SGO. Record: `records/m7-emission-nondivergence-result.md`. |
| (weaken-repair-policy) 2026-09-12 — **no gap, design-verified**: `contrast-collapse` weaken is terminal in code-only campaigns by explicit doctrine (`docs/invariant-challenge.md` v43/D5); design observation on miner-vs-probe threshold asymmetry recorded | CMA + SGO. Design-inspection loop. **Finding**: the pipeline's response to `weaken` from `contrast-collapse` is exactly what M7 exhibits — persist the typed boundary delta, emit no search-policy directive, insert no `invariant_lineage` row, leave the candidate terminal. This is doctrine, not defect. `internal/policy/engine.go:ExpansionBearingDelta` returns `true` only for separation-class deltas (`counterexample-separation`, `constructibility`); bookkeeping-class deltas (`contrast-collapse`, `support-recount`) are explicitly excluded, with `internal/policy/boundary_test.go:53` asserting the exclusion as a regression guard. **SGO cross-check**: M7 has 1 persisted `contrast-collapse` delta and 0 `invariant_lineage` rows; pvnp has 3 persisted `contrast-collapse` deltas and 0 `invariant_lineage` rows — consistent with the doctrine. **Design observation recorded (not a defect)**: the miner emit-gate (`fPrev > sPrev`) and the `success-preserving` probe's zero-cell threshold apply on different scales. The miner admits directional discriminators with sPrev > 0; the probe weakens any predicate with sPrev > 0 at singleton contrast populations. This is exactly correction 7's CMA asymmetry. Code-owned refinement of an `OpEquals` weakened candidate is not designed — refinement requires either a fresh mining pass or a model-authored challenger, both explicitly by doctrine. **Consequence**: no code change warranted. `(weaken-repair-policy)` closed as design-verified, no gap. **H3 gate: preserved.** No admission rules changed, no verifier tiers, no policy directives, no wire schema fields. Pure design inspection + SGO. Record: `records/weaken-repair-policy-result.md`. |
...
| (gow-review-remediation) 2026-09-12 — H1/H2/H3 addressed on `research/l-extension-authoring` for reviewer WITHHOLD at `8f6b1e5` | Code + tests. Three reviewer-flagged findings against the integrated review gate remediated: (a) **GOW-R1 (H2)** — C3 branch rewired to `WitnessCheck→AdmitEvidence` (rule pass), asserting `ObservationDomainCheckedFailure` + `AdmittedBy=="rule"` + recomputation-verified basis; concurrent-writer's edit to `internal/pipeline/review_obligation_integration_test.go` closes the primary assertion. Discriminating negative-control `TestC3NegativeControlAttestationCannotMasqueradeAsIndependentCheck` added — asserts the two label pairs (`domain-checked`/`rule` vs `model-judged`/`operator`) remain distinct so attestation cannot silently satisfy C3. (b) **GOW-R2 (H1)** — three-state compatibility model introduced (`Compatibility = compatible \| stale \| unknown`) in `internal/review/review.go`; `staleAgainstCurrent` → `compatibilityAgainstCurrent` in `internal/pipeline/review_coverage.go` emits `CompatibilityUnknown` when a declared dep has no supplied current ref; projection distinguishes `ReasonStaleDependency` and `ReasonCompatibilityUnknown`. Historical assessments retained regardless of current-decision state. Six-case regression `TestReviewCoverageCompatibilityMatrix` covers complete-match, changed-population, nil-deps, one-omitted, blank-reference, unrelated-extra; two projection-level unit tests `TestProjectUnknownCompatibilityDoesNotGrantCurrentEligibility` and `TestProjectUnknownAndStaleReasonsCoexistWhenBothPresent` pin the reason-code separation. (c) **GOW-R3 (H3)** — `AssessmentRecord` gains `ManifestDependencies` (kind/ref/why-relevant) and `EvidenceCutoff`; `CheckRecord` gains `ProcedureRevision`, `InputsRef`, `Environment`. `from_store.go` carries all fields through the projection adapter. `RenderCoverage` (bumped to `coverage-generator/3`) emits declared-dependencies and per-check procedure-revision/inputs/environment. Regression `TestReviewCoverageExportCarriesCheckerAndDependencyProvenance` asserts every reviewer-flagged field appears verbatim in the rendered document. **Toolchain gates**: `gofmt -l .` empty; `go vet ./...` clean; `go build ./...` clean; `go test ./...` all green (16.007s for `internal/pipeline`, including the concurrent-writer's rewired C3 branch). **NOT closed by this record**: (i) fresh CI execution against branch tip — the reviewer's CI run targets `main@8f6b1e5`; rerunning `.github/workflows/ci.yml` on `research/l-extension-authoring` remains the reviewer's stated verification path; (ii) verbose per-case log capture and retained C1–C8 database/export bundle via `NEWF_REVIEW_COVERAGE_OUT` — reviewer's declared execution limit; (iii) preservation pilot — unrun, requires separate authorization. Record: `records/gow-review-remediation-result.md`. |
| **Recommended next action** | **Revised 2026-09-12 (twenty-fifth revision) after (q) authoring-discipline documentation completed:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh), (l-coh-mechanism-test), (l-population-validation), (source-lineage-check), (m7-emission-nondivergence), (weaken-repair-policy), (source-lineage-admission-gate), (l-coh-arc-consolidation), **(q)** closed for this iteration. **Authoring convention now lives at `docs/proposal-authoring.md`** — decision rule for self-attesting `evaluation_cost` (`low`/`medium`/`high`) with anti-patterns explicitly named. Cross-linked from `docs/frontier-generation.md`. No schema, ranker, or admission changes. **Remaining priorities (needing authorization or authoring)**: **(l-extension)** ⭐: author a third truly-independent corpus in a non-ES, non-P-vs-NP domain, sha256-disjoint from both known lineages. Strongest research-usefulness path. Freeze checklist can now include `newf source lineage-diff` against known lineages and cite `docs/proposal-authoring.md` for the authoring convention. **(es-pvnp-independence)** deferred: cross-corpus independence beyond raw source bytes. Requires (l-extension) or deeper analysis. **(k-full)** Historical pilot re-execution — needs operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families — needs authorization. **(pilot-005-freeze)** Operator authorization on five open items (item #4 substantively complete at code level). **(coherence-metric-pipeline)** Turn (l-coh) measurement into pipeline capability — less compelling given the four-layer evidence for correction 5 and design-verified terminal semantics. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **(gow-review-remediation)** landed on `research/l-extension-authoring`: closes GOW-R1/R2/R3 static findings; **residual advancement condition** = fresh CI rerun on branch tip + verbose per-case log capture + retained C1–C8 bundle via `NEWF_REVIEW_COVERAGE_OUT`. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
