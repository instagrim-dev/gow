...
| **Recommended next action** | **Revised 2026-09-12 (fourth revision) after H3 (evidence-admission attempt) closed path (c):** (a″) and (c) both closed as research routes. Remaining priorities: **(a‴)** Mine an enum-axis or Not-form invariant on the M7 failure population. Highest-leverage — enum/Not predicates are the only shape on this corpus that CAN advance a wire proposal past the H2 + H3 terminal state (they can return `verdict: violated` at the frontier stage AND a decisive `failure` verdict at the evaluation stage). **(e)** Design-space observation about C6 (recorded, not authorised as change; remains for downstream authority). **(b)** Materialise a Pilot-004-specific SQLite fixture — distinct research campaign, unchanged. **(f)** Design-space observation, new: the pipeline's H1/H2/H3 gates form a **three-gate ratchet** — admitted-and-ranked / verifier-cannot-verify / evaluation-blocked. A wire proposal can only pass all three gates when its target invariants have enum-axis or Not-form predicates AND its evaluation reaches a decisive `failure/partial_failure` verdict. Both persisted wire proposals (`fpr_...D2YB1`, `fpr_...TFTE`) are at terminal H3 state and will not advance further without corpus/vocab/verifier evolution. Recording as design-space input, not proposing verifier changes here. Separately: OQ2 remains deferred; N5-child-1 remains blocked on predicate formalization; N2b es-07 inclusion correction still outstanding; atlas `breaks` field inconsistency as an atlas-quality task. |
