...
| **Recommended next action** | **Revised 2026-09-12 (twenty-fourth revision) after (source-lineage-admission-gate) implemented, (pilot-005/openitem-4-status) recorded, and (l-coh-arc-consolidation) authored:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh), (l-coh-mechanism-test), (l-population-validation), (source-lineage-check), (m7-emission-nondivergence), (weaken-repair-policy), **(source-lineage-admission-gate)**, **(l-coh-arc-consolidation)** closed for this iteration. **Canonical arc summary now lives at `records/l-coh-arc-consolidation.md`** — nine reviewer corrections + dispositions + evidence tiers in one place. **The `newf source lineage-diff` diagnostic reproduces the (source-lineage-check) SGO evidence via a single CLI invocation** — `identical` verdict on M7 train vs pilot-001 train (12 shared byte-for-byte), `disjoint` on M7 train vs pvnp train. Admission rules unchanged. Full test coverage: state-space enumeration + same-bytes-different-name discipline + sample-limit + SQL projection. Pilot-005 open item #4 status recorded — code-level substantively complete, remaining gap is one-page operator-facing spec doc (operator's disposition call). **Remaining priorities (needing authorization or authoring)**: **(l-extension)** ⭐: author a third truly-independent corpus in a non-ES, non-P-vs-NP domain, sha256-disjoint from both known lineages. Strongest research-usefulness path. Freeze checklist can now include `newf source lineage-diff` against known lineages. **(es-pvnp-independence)** deferred: cross-corpus independence beyond raw source bytes. Requires (l-extension) or deeper analysis. **(k-full)** Historical pilot re-execution — needs operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families — needs authorization. **(pilot-005-freeze)** Operator authorization on five open items (item #4 substantively complete at code level). **(q)** Authoring-discipline documentation. **(coherence-metric-pipeline)** Turn (l-coh) measurement into pipeline capability — less compelling given the four-layer evidence for correction 5 and design-verified terminal semantics. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
