...
| (o-closed) 2026-09-12 — concrete-falsifiability lever already exists in ranker; no code change required | CMA + SGO. Predeclaration loop for (o) turned into a rescoping. **CMA on `internal/policy/engine.go` + `apply.go`**: (1) every existing directive TargetKind resolves to a persisted-row identity — a directive keyed on free-form falsification-path text would fail the KTD-1 contract; (2) `baseObjectiveLess` ranks candidates as *MechanisticDistance desc → ExpectedInformationGain desc → **EvaluationCost asc** → hash* — concrete-falsifiability is already a first-class tiebreaker at exactly the right position; (3) `computeFalsifiabilityFloor` separately protects the cheapest-to-falsify violating candidate per target invariant. **SGO on M7**: `rank_ordinal` assignments are consistent with `baseObjectiveLess` on `(mech_dist, info_gain, eval_cost)` ordinal tuples; my H2-escape proposal (`evaluation_cost: low`) sits at rank 0 in its violation tier. **The lever is being consumed.** **Verdict**: no code change required. Adding a new directive kind for "non-concrete falsification" would either duplicate the existing `EvaluationCost` signal, rank on falsification-text patterns (KTD-3 forbids model-judgment prose masquerading as code-owned discrimination), or add a wire field that duplicates `evaluation_cost`. **Where the real signal lives**: the (g-scoped) observation is CORPUS-LEVEL — no proposal in the current population is concrete enough for the witness path. The ranker cannot fix an empty concrete-population; only the wire authors (and eventually a mechanism-biased generator) can. **Authoring-discipline observation** (recorded): honest self-attestation of `evaluation_cost: low` for concretely-checkable proposals is the pipeline's designed integration point for the (g-scoped) observation. **Generator-bias observation** (recorded, longer horizon): a generator biased toward mechanisms that produce concrete witnesses (e.g., seeded from es-01 Mordell polynomial identities, es-03 factorization scheme) would produce concrete-witness-path candidates naturally. **H3 gate status: preserved (unchanged).** Record: `records/o-closed-lever-exists.md`. |
...
| **Recommended next action** | **Revised 2026-09-12 (twelfth revision) after (o-closed) confirmed the concrete-falsifiability lever already exists:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o) closed. **All in-pipeline concrete-falsifiability levers now confirmed present and consumed.** Remaining priorities are all research-execution or longer-horizon paths that either (a) exceed a single-loop scope or (b) require authorization to modify historical pilot state: **(l)** Cross-domain replication — materialize pilot-005-relational or another non-ES pilot corpus. Would answer whether the (a‴-B) enum-axis convergence and the (o-lever-already-exists) finding are ES-specific. Corpus construction task; larger scope. **(k-full)** Historical pilot re-execution — atlas-quality re-normalization of 9 unresolved decisive-field claims per historical target, plus operator authorization to modify historical pilot state. Multi-loop. **(p) NEW** Frontier generator bias toward concrete-witness-producing mechanism families (e.g., seed from es-01/es-03) — recorded as design-space observation on (o-closed). Would exercise the witness path for the first time in production. Requires generator code change; needs authorization. **(q) NEW** Authoring-discipline documentation — record that operators authoring wire proposals should self-attest `evaluation_cost: low` when the falsification path is witness-checkable, and be honest about `medium/high` when domain math is required. Prose-only; recorded, not proposed as change. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture (distinct research campaign). **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **The corpus of open code-change paths is essentially empty for the pilot-004 loop; remaining research is either atlas-quality work, corpus materialization, or design-space observations awaiting downstream authority.** Separately: OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
