...
| **Recommended next action** | **Three paths, in priority order (revised 2026-09-12 after pipeline-space discovery): (a) Attack the M7 corpus's actual three surviving invariants (L1/L3/L4-shaped: `identity_carried_solvability`, `confined_to_quadratic_nonresidues`, `class_union_construction`) with a frontier proposal designed against the ACTUAL persisted invariant landscape rather than the paper-space N2a landscape. This produces code-verifiable violation verdicts (not `unknown`), engages the pipeline's admission+ranking+persistence path, and lands on invariants that are in the live corpus. (b) Materialise a Pilot-004-specific SQLite fixture with N2a persisted as `surviving` under mechanism/v4 — a distinct research campaign, not a recipe run. Requires either fresh discovery-and-admission runs or an operator-attested admission based on existing markdown records. This would make the entire paper-space reciprocity-axis chain code-verifiable. (c) Run `newf evidence admit` against the persisted joint-crossing proposal `fpr_01M2B919QNAWFFM5A31KKD2YB1`. Under S2's typed-observation-kind rules the CMA falsification is a `structural-claim-failure` (proposal's own claimed break failed the deterministic discriminant computation) — **never admissible** by rule. This is testable end-to-end now. Separately: OQ2 remains deferred; N5-child-1 remains blocked on predicate formalization; N2b es-07 inclusion correction still outstanding. Corpus-hygiene follow-on from the E15' challenge: atlas's `breaks` field inconsistency remains as an atlas-quality task.** |
