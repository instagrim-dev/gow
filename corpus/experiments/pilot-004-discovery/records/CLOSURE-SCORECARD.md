...
| N2a-child-2 proposed 2026-09-12 | State: `proposed` (Hypothesis-tier). Derived from the falsification above. Claim: any joint-crossing mechanism against N2a's C4 boundary-delta requires a non-degenerate reciprocity carrier (non-square fundamental discriminant with non-trivial genus theory) or equivalent object. Own challenge not yet run. |
...
| **Recommended next action** | **Two paths, in priority order: (a) Challenge N2a-child-2 (does the "non-degenerate reciprocity carrier is required" claim survive its own seven-probe campaign? C1: any atlas success/failure whose mechanism achieves QR-side discrimination via a non-classical apparatus? C2: construct a joint-crossing mechanism whose reciprocity injection is not carrier-based — e.g. via `L`-function twists rather than form genera. C6: is the claim's inductive base one falsified attempt, and is that sufficient support?). (b) Attempt a new frontier proposal against N2a with a rescued reciprocity carrier (e.g. `u² + nv²`, discriminant `−4n`, genuinely non-trivial genus theory) *plus* an argument that it plausibly meshes with the ES linear form — the parent record documents (a rescue analysis, §Rescue attempts) that this is a genuinely new proposal, not a repair. Separately: resolve OQ2 to enable the N3 challenge; challenge N5-child-1 after predicate formalization; materialise a live Pilot-004 SQLite corpus fixture so `newf frontier generate` can run end-to-end. E15 scope correction remains outstanding (OQ4 resolved).** |
