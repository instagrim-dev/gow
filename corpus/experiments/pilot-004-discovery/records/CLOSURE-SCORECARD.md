...
| **Recommended next action** | **Revised 2026-09-12 (twentieth revision) after (source-lineage-check) discharged (l-coh) correction 5 ES-side residual:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh), (l-coh-mechanism-test), (l-population-validation), (source-lineage-check) closed for this iteration. **Three (l-coh) reviewer-corrections now discharged (5 ES-side, 6 arithmetic-verified; 7 upgraded to CMA); five remain at their prior tiers unchanged (1, 2, 3, 4, 8, 9 — note 8/9 are already stable statements, not obligations).** Remaining priorities: **(l-extension)** ⭐ agent-tractable with authoring capacity: author a third truly-independent corpus in a non-ES, non-P-vs-NP domain. The `(source-lineage-check)` result gives a concrete admission criterion — its sha256 set must be disjoint from both the ES and pvnp sets. This is now the strongest remaining research-usefulness path. **(m7-emission-divergence)** ⭐ agent-tractable: investigate why M7's (a‴-B) run persisted only `equals(locality, local)` at `surviving`, when `equals(construction, constructive)` was also emission-eligible per the verified tables. Campaign-run investigation. **(source-lineage-admission-gate)** design-space: turn `(source-lineage-check)`'s sha256-diff into a pipeline corpus-onboarding gate that rejects new corpora sharing sha256 with existing ones (or requires an explicit lineage annotation). Needs authorization. **(es-pvnp-independence)** deferred: the ES-vs-pvnp independence question in a stronger research sense — mutually informative, non-overlapping mechanism spaces, cross-corpus mechanism-fingerprint disjointness beyond raw source bytes. Requires (l-extension) or deeper cross-corpus analysis; not a same-loop discharge. **(k-full)** Historical pilot re-execution — needs operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families — needs authorization. **(pilot-005-freeze)** Operator authorization on six open items. **(q)** Authoring-discipline documentation. **(coherence-metric-pipeline)** Turn (l-coh) measurement into pipeline capability — less compelling now that corrections 5-ES, 6, 7 are all discharged/upgraded. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
