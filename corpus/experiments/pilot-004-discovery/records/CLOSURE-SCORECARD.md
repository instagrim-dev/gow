...
| (l) 2026-09-12 — cross-domain replication executed on P vs NP holdout corpus: **Reading B (partial replication)** | SGO + PE. Fresh isolated `.newf/pvnp/newf.db` at HEAD `324ec5d`. Ingested 12-file pvnp-holdout train (adversarially-decontaminated per `008b0c2`), ran normalize → signature → cluster → failure-space (12 singletons, 5 failure + 7 partial_success), mined with `--emit-posture-axes` (a‴-B flag), challenged with `--all`. **Miner emitted 3 enum-axis candidates**: `equals(construction, existential)` support=2, `equals(uncertainty, deterministic)` support=4, `equals(locality, global)` support=3. All 3 persisted with `association_status=contrast_observed`. **All 3 transitioned to `weaken`** under `success-preserving` probe with `contrast-collapse` delta — success-side preservation cross-cuts posture-axis membership. **Comparison to (i)**: ES corpora produced `equals(locality, local)` at `surviving` with 100% failure-side specificity (0/5 success prevalence); pvnp's `equals(locality, global)` at same support=3 has 3/7 success prevalence, driving `weaken`. **Structural finding (PE)**: enum-axis miner power depends on failure-population coherence in posture-axis space. ES failures form a coherent `local, constructive` cluster; P vs NP failures span the full posture grid (relativization=`global, existential`; monotone LB=`local, constructive`; AC0[p]=`local, constructive`; GCT=`global, constructive-ish`; each barrier program is a structurally different mechanism family). **This refines (i)**: the ES cross-corpus perfect-replication headline is a corpus-property finding about ES's failure-structure homogeneity, not a universal invariant-mining law. **Predeclared Reading B obtained** per `records/l-pvnp-replication-predeclaration.md`; Reading A (surviving) not obtained; Reading C (non-replication) not obtained; Reading D (pipeline failure) not obtained. **H3 gate status: preserved (unchanged).** No admission rules changed, no verifier tier changes, no policy directives, no wire schema fields. This is corpus-execution replication, not an H3 escape attempt. Record: `records/l-pvnp-replication-result.md`. Predeclaration: `records/l-pvnp-replication-predeclaration.md`. **Warrant narrowed 2026-09-12 by (l-coh) — see below.** |
| (l-coh) 2026-09-12 — coherence measurement: predeclared Reading COH-A FALSIFIED; (l) warrant narrowed; (i) data-provenance caveat added | SGO + CMA + PE. Predeclared coherence measure (maxDiscrim ≥ 0.6 for ES) applied to all 5 persisted DBs, then compared with actual miner productivity. **Predeclared Reading COH-A FALSIFIED**: M7's maxDiscrim = **+0.429** at `equals(locality, local)`, below the predeclared 0.6 threshold. pvnp's maxDiscrim = **+0.171**. Ratio ~2.5×; ordering matches (l) PE claim, magnitude weaker. **Reading COH-B OBTAINED**: partial support — direction correct, magnitude overstated. **Data-provenance SGO**: M7/pilot-001/pilot-002 posture distributions are byte-identical under train-filtered queries; pilot-003 is exactly 3× M7. **The four ES corpora are one observation replicated across pipeline HEADs, NOT four independent observations. This substantively weakens the (i) result too — "perfect replication across 4 corpora" becomes "perfect replication of ONE observation across 4 HEADs plus 1 independent domain (pvnp)."** The (i) claim is not falsified but its epistemic strength is lower. Independent-observations count for the (l) hypothesis: **n=2** (ES-shape and P-vs-NP-shape). **Post-hoc hypothesis (flagged, NOT established)**: the sharper discriminator is `minSuccessPrev` over failure-positive `(axis, value)` — M7 = **0.000** (at `equals(locality, local)`), pvnp = **0.286**. This maps directly to `success-preserving` probe semantics (probe returns `confirmed → weaken` as soon as ANY success family preserves). Hypothesis form: *enum-axis mining reaches `surviving` on `equals(axis, value)` iff sPrev(axis, value) = 0*. NOT established because found post-hoc, n=2, would need predeclared replication on a third truly independent corpus. **(l) result correction**: the "coherent cluster vs heterogeneous grid" framing is overstated in magnitude; the actual driver of ES-vs-pvnp `surviving`-vs-`weaken` appears to be a zero-cell effect in the failure × success crosstab, not a discrimination magnitude. **H3 gate status: preserved (unchanged).** No admission rules changed, no verifier tier changes, no policy directives, no wire schema fields — this is SGO measurement on existing SQLite state, not a new pipeline capability. Record: `records/l-coh-measurement-result.md`. Predeclaration: `records/l-coh-measurement-predeclaration.md`. |
...
| **Recommended next action** | **Revised 2026-09-12 (sixteenth revision) after (l-coh) narrowed (l) warrant and added data-provenance caveat to (i):** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh) closed for this iteration. **The (l-coh) measurement falsified predeclared Reading COH-A and added a critical data-provenance caveat to (i): the four ES corpora were byte-identical under train-filter, so the (i) headline "perfect replication across 4 corpora" is one observation replicated across 4 HEADs plus 1 independent domain (pvnp). Independent-observations count for cross-domain claims: n=2, not n=5.** Remaining priorities are research-execution, corpus-materialization, or design-space observations awaiting downstream authority: **(l-extension)** Third truly-independent corpus to move (l) beyond n=2 — requires materializing a new non-ES, non-P-vs-NP domain corpus. **(l-coh-repl)** Predeclare the post-hoc `minSuccessPrev` hypothesis before the third corpus is executed, so the hypothesis can earn or lose epistemic status. **(k-full)** Historical pilot re-execution — multi-loop, atlas-quality re-normalization + operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families (design-space; needs authorization). **(pilot-005-freeze)** Operator authorization on six open items. **(q)** Authoring-discipline documentation — operator-judgment signal. **(coherence-metric-pipeline)** Design-space follow-on: turn (l-coh) measurement into pipeline capability (compute `minSuccessPrev` at mine-time, expose in the miner's output) — needs authorization. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
