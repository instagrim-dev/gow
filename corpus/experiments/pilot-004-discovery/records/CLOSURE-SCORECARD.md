...
| N4 admission gate | **Advanced 2026-09-12:** E15 correction record applied. All three defects addressed — scope narrowing (OQ4), es-05 classification fix (OQ4), and a newly-found defect-3 (contrast_check overreach: E15's "each partial_success records a non-empty break" is falsified within the frozen atlas by es-06 and es-11, both partial_success with `breaks: []`). Revised claim **E15'** (annotation-scope, CMA-verified against the atlas) is separately attributed, in `proposed` state, ready for the seven-probe challenge. Record: `records/E15-correction.md`. |
...
| **Recommended next action** | **Four paths, in priority order: (a) Challenge `E15'` (the corrected N4 claim; scope: annotation pattern for `breaks: []` in es-01/es-03/es-10; C6 should incorporate the newly-discovered fact that `breaks` does not discriminate between `partial_success` and `partial_failure` — es-06 and es-11 are `partial_success` with `breaks: []`). This is now the most-shovel-ready challenge in the queue: bounded, atlas-verifiable, no speculative math. (b) Challenge the sharper `N2a-child-2'` form (enumerates admissible reciprocity layers; falsifiable by a joint-crossing mechanism whose reciprocity layer is *none* of the enumerated types). Substantive testable claim from the prior weakening. (c) Attempt a new frontier proposal against N2a with a non-degenerate reciprocity carrier (e.g. `u² + nv²`, discriminant `−4n`, genuinely non-trivial genus theory) *plus* an argument that it plausibly meshes with the ES linear form. (d) Attempt a frontier proposal that attacks `N2a-child-2'` directly (additive-only / ergodic / character-free apparatus). Separately: resolve OQ2 to enable the N3 challenge; challenge N5-child-1 after predicate formalization; materialise a live Pilot-004 SQLite corpus fixture so `newf frontier generate` can run end-to-end. E15 scope correction is now APPLIED (2026-09-12) — the outstanding items list from prior scorecards no longer includes it.** |
