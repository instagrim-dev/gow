...
| **Recommended next action** | **Revised 2026-09-12 (twenty-second revision) after (weaken-repair-policy) confirmed the pipeline design is intentional and terminal on code-only campaigns:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh), (l-coh-mechanism-test), (l-population-validation), (source-lineage-check), (m7-emission-nondivergence), (weaken-repair-policy) closed for this iteration. **The design observation from (weaken-repair-policy) — miner emit-gate is directional (`fPrev > sPrev`), success-preserving probe is zero-cell binary — is now recorded and confirms correction 7's CMA-tier asymmetry at code-doctrine level.** Remaining priorities (agent-tractable) that do NOT need authorization: **exhausted for this iteration**. All remaining paths need authorization or corpus authoring. **Remaining priorities (needing authorization or authoring)**: **(l-extension)** ⭐: author a third truly-independent corpus in a non-ES, non-P-vs-NP domain, sha256-disjoint from both known lineages. Strongest research-usefulness path. **(source-lineage-admission-gate)** design-space: turn `(source-lineage-check)`'s sha256-diff into a pipeline corpus-onboarding gate. **(es-pvnp-independence)** deferred: cross-corpus independence beyond raw source bytes. Requires (l-extension) or deeper analysis. **(k-full)** Historical pilot re-execution — needs operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families — needs authorization. **(pilot-005-freeze)** Operator authorization on six open items. **(q)** Authoring-discipline documentation. **(coherence-metric-pipeline)** Turn (l-coh) measurement into pipeline capability — less compelling given the three-tier evidence for correction 7 and design-verified terminal semantics. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
