...
| **Recommended next action** | **Revised 2026-09-12 (ninth revision) after (k-restricted) closed pilot-001's specific STOP gate:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted) closed. Remaining priorities: **(g)** Escape H3 — highest-value next research target (H2 escapable but H3 blocks even H2-escaped proposals). Two sub-branches: **(g-A)** A deterministic-check verifier that consumes target-violation verdicts as evidence. **(g-B)** A stronger model-judgment verifier for posture-decisive proposals. **(k-full)** New — full pilot-001 re-execution attempt: pilot-001's `surviving_invariants` gate is now `ready` but `recovery_reachability` is `blocked` under classify/v3. A full attempt requires (a) authorization to freshly certify pilot-001 under the current schema, (b) resolving the recovery_reachability blocker (schema-evolution investigation), and (c) generating new B0/B3 captures — which are epistemically distinct from the historical `prepared_not_captured` outcome. Bigger authorization ask than a normal loop step. **(l)** Cross-domain replication: materialize pilot-005-relational or another non-ES pilot corpus. **(m)** New — investigate the `recovery_reachability` blocker introduced by the classify/v3 schema evolution: what does the check require, why does pilot-001's target fail it, and does this represent a real epistemic gate or a schema-migration issue? Small research task; touches design-space observations about the schema itself. **(e)** Design-space observation about C6 (unchanged; recorded, not authorised). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as a corpus signal (design-space observation). **(a‴-A)** Lower priority — `--proposals-file` on mine. Separately: OQ2 remains deferred; N5-child-1 remains blocked; N2b es-07 inclusion correction still outstanding; atlas `breaks` field inconsistency as an atlas-quality task. |
