...
| **Recommended next action** | **Revised 2026-09-12 (seventeenth revision) after (l-coh) reviewer correction narrowed disposition:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh) closed for this iteration; (l-coh) closes with **narrowed warrant, WITHDRAWN "COH-B obtained" and "n=2 truly independent" and unconditional-`iff` claims**. Remaining priorities (agent-tractable are marked; others need authorization): **(l-coh-mechanism-test)** ⭐ agent-tractable: build two singleton-family fixtures with identical discrimination but different sPrev (A: 5/10 vs 0/10; B: 8/10 vs 3/10; both discrim=0.5), run `VerifySuccessPreserving` on each, verify the probe distinguishes them — this is a code-behavior test upgrading the correction-7 refinement from PE to CMA WITHOUT requiring a third corpus. Reviewer-suggested. **(l-population-validation)** ⭐ agent-tractable: discharge the population-query prerequisite from correction 6 — verify the (l-coh) arithmetic used the eligible families the miner and challenger actually operated on, not merely reachable signatures. **(l-extension)** Author a third truly-independent corpus in a non-ES, non-P-vs-NP domain to move the research-usefulness claim beyond n=2 lineages. Separate experiment from the code-mechanism test. **(k-full)** Historical pilot re-execution — needs operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families — needs authorization. **(pilot-005-freeze)** Operator authorization on six open items. **(q)** Authoring-discipline documentation. **(coherence-metric-pipeline)** Turn (l-coh) measurement into pipeline capability — needs authorization; also needs (l-coh-mechanism-test) result first. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
