...
| **Recommended next action** | **Revised 2026-09-12 (nineteenth revision) after (l-population-validation) discharged (l-coh) correction 6:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped), (o), (witness-scope), (l), (l-coh), (l-coh-mechanism-test), (l-population-validation) closed for this iteration. **Two (l-coh) corrections now discharged (6 arithmetic-verified; 7 upgraded to CMA); seven remain at their prior tiers unchanged.** Remaining priorities (agent-tractable are marked; others need authorization or corpus authoring): **(l-extension)** Third truly-independent corpus in a non-ES, non-P-vs-NP domain — required to move the research-usefulness claim beyond n=2 lineages. This is the strongest remaining path IF a suitable corpus can be authored or found; needs corpus-authoring authorization. **(source-lineage-check)** ⭐ agent-tractable: address the residual half of (l-coh) correction 5 — verify whether the identical marginals across 4 ES corpora reflect literal shared source data or independent inputs that happen to produce identical marginals. Compare `.newf/{m7,pilot-001,pilot-002,pilot-003}/newf.db` at the mechanism-source and normalize-input level (fingerprints, input_set_hash, snapshot IDs). **(m7-emission-divergence)** ⭐ agent-tractable: investigate why M7's (a‴-B) run persisted only `equals(locality, local)` at `surviving`, when `equals(construction, constructive)` was also emission-eligible per the verified tables. Not a correction — a campaign-run investigation. **(k-full)** Historical pilot re-execution — needs operator authorization. **(p)** Frontier generator bias toward concrete-witness-producing mechanism families — needs authorization. **(pilot-005-freeze)** Operator authorization on six open items. **(q)** Authoring-discipline documentation. **(coherence-metric-pipeline)** Turn (l-coh) measurement into pipeline capability — less compelling now that correction 7 is CMA-verified and correction 6 shows the underlying SQL is straightforward. **(n)** Pinned-profile readiness (design-space, unchanged). **(e)** C6 design-space (unchanged). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as corpus signal (design-space, unchanged). **(a‴-A)** Lower priority. **Separately open — NOT discharged**: **C7 obligation** (review-recipe framework; concurrent-writer code changes appear to be addressing). **Witness-occurrence attribution obligation** (F2 gate integration; concurrent-writer addressing). **C8 obligation** (derived coverage projection; concurrent-writer `review_coverage.go` appears to close). OQ2, N5-child-1, N2b es-07, atlas `breaks` field inconsistency remain as atlas-quality tasks. |
