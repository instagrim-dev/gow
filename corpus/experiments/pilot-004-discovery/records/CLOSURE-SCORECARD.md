...
| **Recommended next action** | **Three paths, in priority order: (a) Challenge the sharper `N2a-child-2'` form (enumerates admissible reciprocity layers; falsifiable by a joint-crossing mechanism whose reciprocity layer is *none* of the enumerated types). This is the substantive testable claim the parent's weakening extracted. (b) Attempt a new frontier proposal against N2a with a non-degenerate reciprocity carrier (e.g. `u² + nv²`, discriminant `−4n`, genuinely non-trivial genus theory) *plus* an argument that it plausibly meshes with the ES linear form — the joint-crossing execution record documents this rescue path as a genuinely new proposal, not a repair. (c) Attempt a frontier proposal that attacks N2a-child-2' directly: a joint-crossing mechanism whose apparatus is additive-only, ergodic, or character-free. This is the strongest information-per-effort attack because success falsifies N2a-child-2' and success or failure both discriminate. Separately: resolve OQ2 to enable the N3 challenge; challenge N5-child-1 after predicate formalization; materialise a live Pilot-004 SQLite corpus fixture so `newf frontier generate` can run end-to-end. E15 scope correction remains outstanding (OQ4 resolved).** |
