...
| **Recommended next action** | **Revised 2026-09-12 (eleventh revision) after (g-scoped) reported no matching in-scope obligation and preserved H3:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m), (g-scoped) closed. **The H3 gate is now understood as preserved-by-design, not a next-step target.** Remaining priorities: **(o)** New — search-policy update: downweight frontier proposals whose falsification paths are non-concrete (structural-only, no candidate tuple derivable); upweight proposals whose falsification paths ARE concrete (candidate formula + prime range + tuple attribution). This is a "bounded observation informing search priority while remaining non-decisive about the domain goal" per user authorization on (g-scoped). Small code change to policy engine; requires design-review authorization. **(k-full)** Full historical pilot re-execution — multi-loop, needs authorization + atlas-quality re-normalization of 9 unresolved decisive-field claims per historical target. **(l)** Cross-domain replication: materialize pilot-005-relational or another non-ES pilot corpus. **(n)** Pinned-profile readiness design-space observation (unchanged). **(e)** Design-space observation about C6. **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as a corpus signal. **(a‴-A)** Lower priority — `--proposals-file` on mine. Separately: OQ2 remains deferred; N5-child-1 remains blocked; N2b es-07 inclusion correction still outstanding; atlas `breaks` field inconsistency as an atlas-quality task. Distinct concrete next-loop targets are now (o), (l), and (k-full) — (o) is the smallest and most aligned with the user's explicit sanction. |
