...
| **Recommended next action** | **Revised 2026-09-12 (tenth revision) after (m) diagnosed the corpus-versioning ratchet:** Paths (a″), (c), (a‴-B), (h), (i), (k-restricted), (m) closed. Remaining priorities: **(g)** Escape H3 — highest-value next research target (code change; needs authorization). **(k-full)** Full historical pilot re-execution — now understood to require atlas-quality re-normalization of each historical target (9 unresolved decisive-field claims each) in addition to operator authorization. Larger ask; multi-loop task. **(n)** New (design-space) — pinned-profile readiness: opt-in mode where readiness checks against the classification profile in effect at pilot preparation time, recorded on the manifest. Would separate "would proceed today under today's rules" from "would proceed today under historical rules." Not proposed as change here; recorded for downstream authority. **(l)** Cross-domain replication: materialize pilot-005-relational or another non-ES pilot corpus. **(e)** Design-space observation about C6 (unchanged; recorded, not authorised). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as a corpus signal (design-space observation). **(a‴-A)** Lower priority — `--proposals-file` on mine. Separately: OQ2 remains deferred; N5-child-1 remains blocked; N2b es-07 inclusion correction still outstanding; atlas `breaks` field inconsistency as an atlas-quality task. |
