...
| Frontier proposal 2026-09-12 (against N2a's C4 boundary-delta) | Recorded: **joint-crossing proposal** — δ-method counting identity composed with composition-genus covering, claiming to break both `rate_subthreshold` (es-05) and `qr_confinement` (es-10) in the same mechanism. State: **proposed** (Hypothesis-tier). Wire payload preflighted by `newf experiment validate-proposals` (importer path passed). Not yet routed through `newf frontier generate` (no live SQLite corpus checked in). Not yet falsified — cheapest-path steps 1–4 predeclared, not run. Record: `records/frontier/N2a-joint-crossing-proposal.md`. |
| Cluster revision under v5 | `clr_01M28W6AF7DBAX5363TVXSN3NR` (12 singletons, clean) |
| **Recommended next action** | **Execute the joint-crossing proposal's cheapest-falsification-path steps 1–2 (literature check on Bombieri–Vinogradov admissibility; composition-genus non-emptiness computation for Q(u,v) = uv + λn(u+v) over QR-survivor classes for small operative primes). Both bounded to hours. Steps 3–4 (days) follow if steps 1–2 do not falsify. Separately: resolve OQ2 to enable the N3 challenge; challenge N5-child-1 after predicate formalization; materialise a live Pilot-004 SQLite corpus fixture so `newf frontier generate` can run against N2a end-to-end (currently no live corpus is checked in — the proposal was preflighted through the shared importer path but not persisted to a generation record). E15 scope correction remains outstanding (OQ4 resolved).** |
