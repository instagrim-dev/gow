...
| **Recommended next action** | **Revised 2026-09-12 (eighth revision) after (i) confirmed cross-corpus convergence on 4 ES pilots with headline zero→two on pilot-001:** Paths (a″), (c), (a‴-B), (h), (i) closed. Remaining priorities: **(g)** Escape H3 — highest-value next research target (H2 escapable but H3 blocks even H2-escaped proposals). Two sub-branches: **(g-A)** A deterministic-check verifier that consumes target-violation verdicts as evidence. **(g-B)** A stronger model-judgment verifier for posture-decisive proposals. **(k)** New — retroactively enrich historical pilots' invariant revisions and re-execute their downstream challenge and frontier stages with the additional `contrast_observed` invariants now available. Would produce a comparative before/after: does pilot-001's research outcome change when it has invariants at all? Do pilots 2 and 3's frontiers rank differently when they can target `equals(locality, local)` instead of the weaker `contains(preserves, X)` invariants? Not a code change; a research-execution task. Requires user authorization for durable modification of historical pilot state. **(l)** New — cross-domain replication: materialize pilot-005-relational or another non-ES pilot corpus and check whether the enum-axis convergence holds outside ES. Answers whether the (a‴-B) extension is ES-specific or a general routing capability. **(e)** Design-space observation about C6 (unchanged; recorded, not authorised). **(b)** Materialise a Pilot-004-specific SQLite fixture. **(j)** Axis co-variation as a corpus signal (design-space observation). **(a‴-A)** Lower priority — `--proposals-file` on mine. Separately: OQ2 remains deferred; N5-child-1 remains blocked; N2b es-07 inclusion correction still outstanding; atlas `breaks` field inconsistency as an atlas-quality task. |
