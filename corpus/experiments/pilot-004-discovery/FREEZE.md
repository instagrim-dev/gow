# Pilot 004 — FREEZE (operator attestation)

Attestor: operator (attestation supplied as session token 'confirmed' in the
driving session; recorded by the executing agent — this documents the actual
form of the attestation, per the pilot-003 precedent of accepting
attestations at their actual time and scope)
Attested at (UTC): 2026-09-11T12:06:23.753149+00:00

Committing this file attests: the protocol below is the frozen design; the
resolved decisions D1-D4 stand; the assembled inputs are exactly the digests
below; the reference standard is the pinned pilot-001 ledger. No capture was
dispatched before this file was committed.
...
Capture order: three D1 runs, each sealed (validator + transcript audit +
byte-exact extraction) before the next dispatch; then three D0 runs
likewise. Pre-declared exception: harness status emissions
(UpdateCurrentStep) do not void a capture; ANY other tool use beyond the
single sanctioned prompt-file Read does. Adjudication is operator-owned
(decision D3): the executing agent stops after sealing and ledger assembly.
