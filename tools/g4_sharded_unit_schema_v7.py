#!/usr/bin/env python3
"""Validate public G4 typed-history route-selection v7 schemas."""
...
def validate_intent(unit: object, response_schema: dict[str, Any]) -> None:
    ...
    for index, item in enumerate(unit["episode"]["history"]):
        _validate_history_expression(item["start"], "$.episode.history[" + str(index) + "].start", declared)
...
def validate_materialized(unit: object, assignment: dict[str, str]) -> None:
    ...
    if unit.get("construction_boundary") != "model_authored_typed_history_start_host_validated_and_canonicalized_model_selected_versioned_recipe_and_endpoint_host_expanded_route_only":
    ...
    for index, step in enumerate(steps):
        ...
        if not isinstance(step, dict) or step.get("direction") != "forward" or step.get("construction_origin") != "host_expanded_from_model_recipe_selection":
