#!/usr/bin/env python3
"""Public end-to-end discriminator for G4 typed-history authoring v7."""
...
from g4_sharded_authoring_v7 import build_request, invoke, run  # noqa: E402
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    packet.mkdir()
    for name in (
        "G4_SHARDED_UNIT_REQUEST_TEMPLATE_V7.json",
        "G4_SHARDED_AUTHORING_ENVELOPE_V7.md",
        "G4_ROUTE_SELECTION_SPEC_V7.md",
        "G4_TYPED_HISTORY_SPEC_V7.md",
    ):
        source = root / "docs" / "plans" / name
        (packet / name).write_bytes(source.read_bytes())
    plan_path = root / "docs" / "plans" / "G4_SHARDED_AUTHORING_PLAN_V7.json"
    ...
    assert "typed-history route-selection" in request_text
    ...
    for recipe in RECIPES:
        ...
        assert materialized["construction_boundary"] == "model_authored_typed_history_start_host_validated_and_canonicalized_model_selected_versioned_recipe_and_endpoint_host_expanded_route_only"
        assert materialized["episode"]["history"][0]["start"] == "add(0, x)"
        assert len(materialized["route"]["steps"]) >= 2
        assert all(step["construction_origin"] == "host_expanded_from_model_recipe_selection" for step in materialized["route"]["steps"])
    ...
    raw_history = intent_for(assignment)
    raw_history["episode"]["history"][0]["start"] = "add(0, x)"
    code, receipt, staging = invoke_case("raw-history-string", raw_history)
    assert code == 2 and receipt["status"] == "rejected_before_staging" and not list(staging.iterdir())
    ...
    assert terminal["status"] == "authoring_staged" and terminal["attempted_invocations"] == 24 and terminal["validated_staged_units"] == 24
    assembled = json.loads((attempt / "assembled-staging.json").read_text())
    assert assembled["schema"] == "g4-authoring-assembled-staging/7" and assembled["unit_count"] == 24
    assert {unit["episode"]["family"] for unit in assembled["units"]} == {
        "protected-inf-a", "protected-inf-b", "protected-low", "protected-misleading",
    }
...
print(json.dumps({
    "schema": "g4-typed-history-public-discriminator/1",
    "schedule": {
        "invocations": 24,
        "per_tokens": 2048,
        "per_seconds": 45,
    ...
    "positive": "complete_24_unit_assembly",
    "interruption": "five_staged_no_assembly",
    "deadlines": "slow_stream_and_slow_materializer_terminated_without_staging",
    "request_inputs": "v7_envelope_template_selection_and_typed_history_specs_only",
}))
