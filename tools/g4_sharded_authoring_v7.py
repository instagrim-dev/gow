#!/usr/bin/env python3
"""Public host-side transport and staging for G4 typed-history selection v7.

This program does not authorize an invocation.  Its caller supplies the
packet-local endpoint, model, limits, and protected locations only after those
facts have been authorized.  It treats every response as data: incomplete,
...
from g4_sharded_unit_schema_v7 import load_materialized, validate_intent, validate_materialized
...
def build_request(packet: Path, assignment: dict[str, str], model: str, max_tokens: int) -> dict[str, Any]:
    ...
    template = load_materialized(str(packet / "G4_SHARDED_UNIT_REQUEST_TEMPLATE_V7.json"), assignment)
    instruction = template.get("instruction")
    if not isinstance(instruction, str) or not instruction:
        raise Reject("invalid_unit_template")
    route_spec = (packet / "G4_ROUTE_SELECTION_SPEC_V7.md").read_text(encoding="utf-8")
    history_spec = (packet / "G4_TYPED_HISTORY_SPEC_V7.md").read_text(encoding="utf-8")
    envelope = (packet / "G4_SHARDED_AUTHORING_ENVELOPE_V7.md").read_text(encoding="utf-8")
    return {
        "model": model,
        "stream": False,
        "keep_alive": "0s",
    ...
    *,
    ...
    try:
        ...
        _write_json(
            ...
            {"schema": "g4-sharded-unit-invocation/7", "status": "transport_failed", "tool_schema_supplied": False, "host_tool_dispatch": "absent", "reason": type(error).__name__},
            exclusive=True,
        )
        return 2
    finally:
    ...
    receipt: dict[str, Any] = {
        "schema": "g4-sharded-unit-invocation/7",
...
def _validate_plan(plan: Any) -> list[dict[str, str]]:
    if not isinstance(plan, dict) or plan.get("schema") != "g4-sharded-authoring-plan/7" or plan.get("unit_schema") != "g4-authoring-unit-intent/5" or plan.get("materialized_unit_schema") != "g4-authoring-unit-materialized/1":
        raise Reject("invalid_plan_schema")
    units = plan.get("units")
    if not isinstance(units, list) or plan.get("unit_count") != len(units) or len(units) != 24:
        raise Reject("invalid_plan_population")
    expected = Counter({"history_informative": 12, "history_low_value": 6, "history_misleading": 6})
    if any(
    ...
    expected_families = (["protected-inf-a"] * 6 + ["protected-inf-b"] * 6 + ["protected-low"] * 6 + ["protected-misleading"] * 6)
...
def _validate_run_plan(plan: Any) -> list[dict[str, str]]:
    ...
    if plan.get("generation_order") != "unit-00-through-unit-23" or [assignment["id"] for assignment in assignments] != expected_assignments:
        raise Reject("generation_order_does_not_match_frozen_schedule")
    if per != {"max_tokens": 2048, "max_elapsed_seconds": 45, "retries": 0} or aggregate != {"max_invocations": 24, "max_tokens": 49152, "max_elapsed_seconds": 1080}:
...
def run(
    *,
    ...
    for assignment in assignments:
        ...
        _append_json_line(progress_path, entry)
    try:
        result = assembler(plan_path=plan_path, template_path=packet / "G4_SHARDED_UNIT_REQUEST_TEMPLATE_V7.json", staging=staging)
        _atomic_write_json(attempt / "assembled-staging.json", result)
    ...
    _append_json_line(progress_path, {"schema": "g4-sharded-authoring-progress/1", "unit_id": None, "status": "assembled_staging", "attempted_invocations": attempted, "validated_staged_units": staged, "aggregate_elapsed_seconds": round(time.monotonic() - started, 3), "protected_response_contents_inspected": False})
    return terminal("authoring_staged", None, "complete_population_assembled_staging_only", assembled=True)
...
def assemble(*, plan_path: Path, template_path: Path, staging: Path) -> dict[str, Any]:
    ...
    for assignment in assignments:
        ...
        units.append(unit)
    return {
        "schema": "g4-authoring-assembled-staging/7",
